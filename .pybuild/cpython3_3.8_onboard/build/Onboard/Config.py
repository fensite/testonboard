...
class Config(ConfigObject):
    ...
    def is_docking_enabled(self):
        # 判断文件存在不存在设置个setting值
        # os.path.exists(test_file.txt)
        ini_docking_enable_file = os.path.join(xdg.xdg_config_home,"docking_enable")
        if not os.path.exists(ini_docking_enable_file):
            with open(ini_docking_enable_file,'w') as docking_enable_isexists:
                docking_enable_isexists.write("exists")
                os.popen("gsettings set org.onboard.window docking-enabled false")
